...
| T3 | 文档管理 UI（列表 + 上传 + 删除） | ✅ 已完成 | T2 | #135 |
| T4 | 会话管理 UI（列表 + 创建 + 删除 + 切换） | ✅ 已完成 | T3 | #137 |
| T5 | SSE 流式问答 + 消息渲染 + 引用卡片 + 复制按钮 | ✅ 已完成 | T3, T4 | #139 |
| T6 | 服务器历史恢复 + 既有会话续聊 | ✅ 已完成 | T5 | #141 |
| T7 | 反馈 UI（点赞/点踩 + 历史消息反馈） | 待实施 | T6 | 待创建 |
| T8 | 设置页 + API Key 管理 + 后端 /web 路由 + docker 集成 | 待实施 | T7 | 待创建 |
...
- **状态**：✅ 已完成（Issue #135）
- **目标**：端到端接通现有文档 API，在 React SPA 中完成文档列表、单 PDF 上传、删除和状态展示
- **技术方案**：
  - 引入 TanStack Query，并新增 `useDocuments` / `useUploadDocument` / `useDeleteDocument` hooks；上传或删除成功后统一失效文档查询缓存
  - `ApiClient` 新增原生 `FormData` 上传与 204 删除接口；400、404、500 错误继续以 `ApiClientError` 暴露状态码和后端详情
  - 左侧文档区默认展开、支持折叠，展示文件名、页数和 `pending` / `processing` / `ready` / `failed` 四种状态；失败项展示后端原因
  - 主区底部输入栏新增单 PDF 文件选择器与上传按钮，上传期间禁用并展示明确状态；非 PDF 和服务端错误显示友好提示
  - 删除不显示二次确认；成功后刷新文档列表
  - 小屏保持现有功能，以侧栏和主区上下分区避免内容重叠；完整移动导航交互留待后续独立模块
- **验收**：46 个前端测试全绿，其中 13 个聚焦 T3 行为；前端 lint / test / build 与后端 CI 全量检查通过

### T4 会话管理 UI（列表 + 创建 + 删除 + 切换）

- **状态**：✅ 已完成（Issue #137）
- **目标**：端到端接通现有会话 API，在 React SPA 中完成会话列表、创建、删除、切换和会话级文档范围锁定
- **技术方案**：
  - `ApiClient` 新增会话列表、JSON 创建和 204 删除接口；`useConversations` 系列 hooks 统一管理查询、mutation 与缓存失效
  - 历史会话区默认展开且可折叠，覆盖加载、空和错误状态；会话项显示标题、创建日期与锁定文档数量
  - `App` 持有当前会话和选中文档状态；切换或创建后同步侧栏高亮与主区会话摘要，删除当前会话后清空当前选择
  - 仅 `ready` 文档可勾选；创建会话时把选中 ID 作为 `document_ids` 快照提交，空数组表示全部文档
  - 创建和删除均无确认弹窗，成功后刷新会话列表；列表与 mutation 错误使用统一友好提示
  - 会话行和文档行使用稳定网格轨道，桌面与窄屏均保持操作控件可见且无内容挤压
  - 400/422 与未分类 HTTP 错误只显示统一友好提示，不暴露后端 detail；长会话摘要在窄屏安全省略并保留完整悬浮提示
- **验收**：67 个前端测试全绿，其中 21 个为 T4 新增行为测试；前端 lint / test / build 本地通过

### T5 SSE 流式问答 + 当前消息渲染 + 可核查引用 + 复制

- **状态**：✅ 已完成（Issue #139）
- **目标**：在本页新建的会话中接通 SSE 问答，支持当前页面内连续多轮、可核查引用和带来源复制；既有会话历史恢复留给 T6
- **技术方案**：
  - `ApiClient.askQuestionStream` 通过原生 `fetch` + `ReadableStream` POST `/api/v1/queries`，强制 `stream: true` 并复用认证头与 `AbortSignal`
  - SSE 解析覆盖任意字节分块、跨块 UTF-8、LF/CRLF 与尾部缓冲；非法事件、服务端 `error`、缺失 `done` 或不可读 body 统一进入失败语义
  - `useChat` 按本页会话保存临时 transcript；发送立即追加 Turn，token 实时追加，done 写入引用与 `request_id`；若后端提供 `message_id` 则同步替换本地 ID
  - 仅本页新建会话可发送；停止、切换或删除会话会 abort 并回滚未完成 Turn，手动停止与失败会还原问题
  - assistant 使用 `react-markdown` + `remark-gfm` 渲染；引用保持正文真实 `C<n>` 编号，可定位高亮卡片，卡片提供文档名、页码、三行预览与全文展开
  - 完成回答提供图标复制按钮，复制纯文本正文并追加简明来源表；样式覆盖桌面双列与 390px 窄屏单列引用卡片
- **验收**：90 个前端测试全绿；前端 lint / test / build 与 `git diff --check` 本地通过；桌面与 390x844 视觉检查通过

### T6 服务器历史恢复 + 既有会话续聊

- **状态**：✅ 已完成（Issue #141）
- **目标**：以服务端历史为权威恢复既有会话，支持多会话后台并行生成和既有会话续聊
- **技术方案**：
  - 选择会话后通过 TanStack Query 加载详情；缓存历史立即显示并后台重新验证，验证成功前禁止发送，加载失败可重试
  - `App` 持有单个共享聊天协调器，按 Conversation 隔离临时 Turn、草稿、请求控制器和错误状态；不同 Conversation 可并行，同一 Conversation 最多一个进行中 Turn
  - 切换会话不终止后台流；停止只中止当前会话并恢复其问题，删除生成中会话先中止并丢弃临时 Turn
  - 流结束后按 `done.request_id` 回读详情确认持久化；服务端消息替换临时 Turn 并刷新会话标题与最近活跃排序
  - 同步失败保留临时答案、禁止继续发送并提供重试；重新验证发现相同 `request_id` 时自动采用服务端历史并去重
  - 侧栏用独立图标显示每个 Conversation 的生成中、同步失败和生成失败状态
- **刷新边界**：硬刷新不恢复上次选择，并中止本页所有流式请求；重新进入后只展示已成功持久化的服务端历史
- **后续优化**：跨刷新继续生成。需要后端引入可持久化任务 ID、任务状态查询和 SSE 重连/结果恢复协议，独立 Issue 实施，不纳入 T6
- **验收**：111 个前端测试全绿；前端 lint / test / build 与 `git diff --check` 本地通过；桌面与 390x844 浏览器交互和视觉检查通过
