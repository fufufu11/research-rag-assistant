...
- **已覆盖阶段**：阶段 0-7（基础功能）+ 阶段 8.1（Reranker 重排序）
- **测试**：273 个非 API 单元测试通过（API 测试需 Qdrant，CI 上全绿）
- **CI**：ruff format + ruff check + mypy + pytest 三项全绿
- **评测**：Hit@1=50%、Hit@5=70%、MRR=0.590（chunk-500-overlap-0 + reranker）
...
- **风险**：页码溯源需重新设计（chunk 可能跨多页，需记录起始页+结束页）
- **验收**：跨页问题不再失败，页码引用仍准确
